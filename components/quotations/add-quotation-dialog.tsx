...
interface AddQuotationDialogProps { clients: Client[] }

// ─── Component ────────────────────────────────────────────────────────────────

export function AddQuotationDialog({ clients: initialClients }: AddQuotationDialogProps) {
  const [open, setOpen] = useState(false)
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <Label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Quotation #</Label>
              <div className="h-9 flex items-center px-3 rounded-md border border-[#00BCD4] bg-[#00BCD4]/10 text-[#00BCD4] font-bold text-sm">{quoteNumber}</div>
            </div>
            <div className="space-y-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <Label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Valid Until</Label>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button type="submit" disabled={loading} className="bg-[#FF6B00] hover:bg-[#FF6B00]/90 text-white font-semibold px-6">
              {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating...</> : 'Create Quotation'}
            </Button>
          </div>
        </form>
      </DialogContent>
